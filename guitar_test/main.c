...
void read_chart(song_t *s,  sync_track_t *syn, event_t *ev, single_t *notes, FRESULT rc, DIR dir, FILINFO fno, char* song){
  //song = strcat("/charts/","notes.txt\0");
...
void draw_game(int game_ctl,int* game_drawn, fret_t frets[20], int d_rate, int* counter, int* countdown, single_t *notes){
  if(game_ctl == 0){
    if(!(*game_drawn)){
      f3d_lcd_fillScreen(BLACK);
      //frets
