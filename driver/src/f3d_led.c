...
void f3d_led_init(void) {
  //must initialize clock for GPIO port before assigning
  RCC_AHBPeriphClockCmd(RCC_AHBPeriph_GPIOE, ENABLE);

  GPIO_InitTypeDef GPIO_InitStructure;
  GPIO_StructInit(&GPIO_InitStructure);
  //each pin is a bit, choose led pins and bitwise or to include only allowable fields
...
void f3d_led_on(int led) {
  //must take vals 0-7. Bits are aligned so just shift by led.
  //BSRR is on
